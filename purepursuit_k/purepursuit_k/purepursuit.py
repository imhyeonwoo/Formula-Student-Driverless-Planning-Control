#!/usr/bin/env python3
import rclpy
from rclpy.node import Node
from std_msgs.msg import Float32            # ← 변경: Float32MultiArray → Float32
import math
import numpy as np
from nav_msgs.msg import Path
from visualization_msgs.msg import Marker
from std_msgs.msg import ColorRGBA
from geometry_msgs.msg import Point

#---- Vehicle Params ----
MAX_STEERING = 45       # [deg]
CAR_POSITION = (0.0, 0.0)
PATH_INTERVAL = 0.2

class PurePursuitControl(Node):
    def __init__(self):
        super().__init__('PurePursuitControl')

        # Params
        self.declare_parameter('LAD', 2.0)
        self.declare_parameter('WHEELBASE', 1.3)
        self.declare_parameter('SPEED', 10.0)

        self.LAD = float(self.get_parameter('LAD').value)
        self.WHEELBASE = float(self.get_parameter('WHEELBASE').value)
        self.SPEED = float(self.get_parameter('SPEED').value)

        # Subscribers
        self.create_subscription(Path, '/local_planned_path', self.callback_path, 10)
        self.create_subscription(Float32, '/current_speed', self.callback_currentspeed, 10)  # ← 변경

        # Publishers (Float32로 통일)
        self.pub_speed = self.create_publisher(Float32, '/cmd/speed', 10)
        self.pub_steer = self.create_publisher(Float32, '/cmd/steer', 10)

        # Visualize
        self.pub_lad = self.create_publisher(Marker, '/lookahead_point_marker', 1)

        # State
        self.path = []
        self.current_speed = 0.0

        # Timer
        self.timer = self.create_timer(0.1, self.main_loop)

        self.get_logger().info('----- Pure Pursuit Control Started -----')

    def callback_path(self, msg: Path):
        path_xy = []
        poses = msg.poses
        if not poses:
            self.path = []
            return
        last = None
        for ps in poses:
            x = float(ps.pose.position.x)
            y = float(ps.pose.position.y)
            if not (math.isfinite(x) and math.isfinite(y)):
                continue
            if last is None or math.hypot(x - last[0], y - last[1]) >= PATH_INTERVAL:
                path_xy.append((x, y))
                last = (x, y)
        self.path = path_xy

    def callback_currentspeed(self, msg: Float32):
        self.current_speed = float(msg.data)

    def find_target(self, lad: float):
        if not self.path:
            self.get_logger().warn("Path is empty. Returning (0.0, 0.0) as target.")
            return (0.0, 0.0)
        target = None
        min_err = float('inf')
        for (x, y) in self.path:
            distance = math.hypot(x, y)
            err = abs(lad - distance)
            if err < min_err:
                min_err = err
                target = (x, y)
        return target if target is not None else (0.0, 0.0)

    def compute_steering(self, target):
        dx = target[0] - CAR_POSITION[0]
        dy = target[1] - CAR_POSITION[1]
        alpha = math.atan2(dy, dx)
        steering = math.atan2(2.0 * self.WHEELBASE * math.sin(alpha), math.hypot(dx, dy))
        return steering

    def drive(self, steering, speed):
        steer_msg = Float32()
        steer_msg.data = float(steering)
        self.pub_steer.publish(steer_msg)

        speed_msg = Float32()
        speed_msg.data = float(speed)
        self.pub_speed.publish(speed_msg)

    def vis_lad(self, lad: float):
        if lad is None or lad <= 0:
            return
        marker = Marker()
        marker.header.frame_id = 'base_link'
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.ns = 'lad_semicircle'
        marker.id = 1
        marker.type = Marker.LINE_STRIP
        marker.action = Marker.ADD
        marker.scale.x = 0.03
        marker.color = ColorRGBA(r=0.0, g=1.0, b=1.0, a=1.0)
        marker.pose.orientation.w = 1.0
        cx, cy = CAR_POSITION
        num_pts = 64
        for theta in np.linspace(-math.pi/2, math.pi/2, num_pts):
            x = cx + lad * math.cos(theta)
            y = cy + lad * math.sin(theta)
            marker.points.append(Point(x=x, y=y, z=0.0))
        self.pub_lad.publish(marker)

    def main_loop(self):
        if not self.path:
            return

        lad = float(self.LAD)
        target = self.find_target(lad)
        steering_angle = self.compute_steering(target)
        steering_angle = float(np.clip(steering_angle,
                                       math.radians(-MAX_STEERING),
                                       math.radians(MAX_STEERING)))
        # publish steering angle in degrees
        steering_angle_deg = math.degrees(steering_angle)
        speed = float(self.SPEED)
        self.drive(steering_angle_deg, speed)
        self.vis_lad(lad)

def main(args=None):
    rclpy.init(args=args)
    node = PurePursuitControl()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()
        rclpy.shutdown()

if __name__ == '__main__':
    main()
