...
# 각도 티어(벽 라인용)
# PARENT_DEG_TIERS = [65.0, 95.0]   # STERRING COURSE
PARENT_DEG_TIERS = [30.0, 95.0]   # ACCELERATION COURSE
...
# 색상 키/표시 색
LEFT_KEYS  = {'blue', 'bluecone'} #{'yellow', 'yellowcone'}
RIGHT_KEYS = {'yellow', 'yellowcone'} #{'blue', 'bluecone'}
RED_KEYS   = {'red', 'redcone'}
LEFT_COLOR   = (0.0, 0.3, 1.0, 1.0) #(1.0, 1.0, 0.0, 1.0)
RIGHT_COLOR  = (1.0, 1.0, 0.0, 1.0) #(0.0, 0.3, 1.0, 1.0)
...
class ConesColorSubscriber(Node):
    ...
    def _greedy_path(self, start, graph, pts, keys, prefer_keys) -> List[int]:
        ...
        max_levels = min(MAX_RELAX_LEVELS,
                        max(len(self.parent_cos), len(self.grand_cos), len(self.global_cos) or [1]))
